...
export function AgrovetFarmersScreen() {
  ...
  const filtered = useMemo(
    () => db.farmers.filter((item) => `${item.name} ${item.nationalId}`.toLowerCase().includes(search.toLowerCase())),
    [db.farmers, search],
  );
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}> 
      <TopAppBar
        title={mode === 'list' ? 'Farmers List' : mode === 'detail' ? 'Farmer Details' : selected ? 'Edit Farmer' : 'Add New Farmer'}
        subtitle="Stored locally for offline use"
        showBack={mode !== 'list'}
        onBackPress={() => setMode('list')}
        actions={mode === 'list' ? [
          { icon: 'add-outline', onPress: () => setMode('add'), accessibilityLabel: 'Add Farmer' },
      ...
      {mode === 'list' ? (
        <View style={styles.content}>
          <SearchBar value={search} onChangeText={setSearch} placeholder="Search by name or national ID" />
          <FlatList
            data={filtered}
      ...
      {mode === 'detail' && selected ? (
        <View style={styles.content}>
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            ...
            <Text style={[styles.text, { color: colors.grayMuted }]}>Updated: {new Date(selected.updatedAt).toLocaleString()}</Text>
          </View>
          <PrimaryButton label="Edit Farmer" onPress={() => { setForm({ name: selected.name, phone: selected.phone, nationalId: selected.nationalId, location: selected.location, sacco: selected.sacco }); setMode('add'); }} />
          <PrimaryButton label="Delete Farmer" onPress={() => { deleteFarmer(selected.id); setSelected(null); setMode('list'); }} variant="outline" />
        </View>
      ) : null}
      ...
      {mode === 'add' ? (
        <View style={styles.content}>
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}> 
            ...
            <TextInput value={form.name} onChangeText={(v) => setForm((p) => ({ ...p, name: v }))} placeholder="e.g., Wanjiku" placeholderTextColor={colors.grayMedium} style={[styles.input, { borderColor: colors.border, color: colors.text }]} />
            <Text style={[styles.label, { color: colors.text }]}>Phone</Text>
            <TextInput value={form.phone} onChangeText={(v) => setForm((p) => ({ ...p, phone: v }))} placeholder="e.g., +254712000111" placeholderTextColor={colors.grayMedium} style={[styles.input, { borderColor: colors.border, color: colors.text }]} />
            <Text style={[styles.label, { color: colors.text }]}>National ID</Text>
            <TextInput value={form.nationalId} onChangeText={(v) => setForm((p) => ({ ...p, nationalId: v }))} placeholder="e.g., 31245678" placeholderTextColor={colors.grayMedium} style={[styles.input, { borderColor: colors.border, color: colors.text }]} />
          </View>
